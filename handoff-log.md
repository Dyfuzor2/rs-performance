...
[2026-04-12 ~evening CET] Cursor -> **obraz sytuacji prod + ergonomia SSH:** Zweryfikowano SSH na **hostingu** (hasło operatora — tylko sesja / `mcp.env`, nie repo): Laravel **13.1.1**, PHP **8.5.3**, `public_html/.well-known/*` (A2A/AEO artefakty), trasy A2A (`message:send` itd.), curl z serwera na canonical: browser **200**, GPTBot **302**, `a2a.json` **200**. **VPS:** n8n-mcp/n8n/auto health **200**, bramka `/for-agents` **200**. **Repo:** `ssh_exec.py` — przed połączeniem wywołuje `load_cursor_env()` z `.cursor/mcp.env`, żeby `CYBERFOLKS_SSH_*` było obok pozostałych sekretów (gitignored); zaktualizowano `.cursor/mcp.env.example`; dodano `docs/ops-production-verification-snapshot.md` (fakty bez haseł). **Operator:** wstaw `CYBERFOLKS_SSH_PASSWORD` do `.cursor/mcp.env` — nie powtarzaj hasła w czacie; rotuj jeśli wyciekło.

[2026-04-12 ~18:35 CET] Cursor -> hosting live: **.htaccess — synchronizacja UA z `config/ai_agents.php`** — w regułach ModSecurity **99000/99001** oraz w `RewriteCond` bramki WOW brakowało tokenów obecnych w katalogu PHP (m.in. **Exabot**, **OpenRouter**, **BingPreview**, **HuggingFaceBot**, **RedditBot**, **ShapBot**, **ScaleAI-Crawler**, **Together-Bot**, **cohere-crawler**, **cohere-training-data-crawler**, **hugging-face-ai**). Skutek: zaproszone agenty mogły dostawać **406** zamiast bypassu / **302** na `ai.rsperformance.online`. Backup: `public_html/.htaccess.bak_cursor_aitokens_20260412`. Deploy: `ssh_exec.py --upload` `.htaccess_remote` → `public_html/.htaccess`. Weryfikacja: `curl -I -A "…Exabot…" https://rsperformance.online/` → **302** → bramka; `scripts/smoke_ai_agent_catalog.py` → **0** zaproszonych z **406** na `/`. Repo: commit `3502630` na `feature/v9-architecture-rebuild`. **VPS:** bez zmian (trasy/health już OK).

[2026-04-12 ~18:50 CET] Cursor -> **relay post-flight:** zaktualizowano `HANDOFF.md` (sekcja górna + Ostatni agent + Następne kroki), `plan.md` (current status), `start.md` (blok LIVE), `SESSION_LOG.md` (wpis sesji). Push: `git push origin feature/v9-architecture-rebuild` → `9275960..ecf7bcd` (w tym `chore(relay): handoff after htaccess ai_agents UA parity deploy`).

[2026-04-12 ~late CET] Cursor -> hosting live: **n8n WOW „Wyzwól” — auto-webhook + diagnostyka** — na hostingu **brak `.git`**, deploy **SFTP** (`ssh_exec.py --upload`) do `/home/tyurjydtpw/domains/rsperformance.online/laravel/`: `app/Support/N8n/N8nWorkflowWebhookPathResolver.php` (nowy), `N8nWorkflowExecuteService.php`, `Filament/Resources/N8nWorkflowDocumentResource.php`, `config/n8n.php` (m.in. `auto_resolve_webhook_path`). Backupy przed podmianą: `*.bak_cursor_n8n_aw_20260412`. Komendy: `composer dump-autoload -o`, `php85 artisan config:clear`, `php85 artisan optimize:clear`, `php85 artisan migrate --force` → **Nothing to migrate**. Weryfikacja z serwera: `php85 -l` na 4 plikach → OK; `curl` `https://rsperformance.online/` i `/admin/login` → **200**. Repo: commit `ae48563` na `feature/v9-architecture-rebuild` (wielokrotne webhooki, retry GET↔POST przy 405, `Auto-webhook:` w komunikatach).

[2026-04-12 ~21:50 CET] Cursor -> hosting live: **n8n webhook path resolver + WOW hub + blog config** — SFTP `ssh_exec.py --upload` → `domains/rsperformance.online/laravel/`: `N8nWorkflowWebhookPathResolver.php` (ścieżki z `:param`, `workflow.nodes`, hint „jest Webhook ale ścieżka…”), `N8nPublicApiHealthService.php`, `N8nWorkflowDocumentResource.php`, `N8nWorkflowDocumentResource/Pages/ListN8nWorkflowDocuments.php`, `Widgets/N8nWorkflowHubOverview.php`, `config/blog.php` (`php_cli_binary` / `BLOG_PHP_CLI`). Backupy: `*.bak_cursor_20260412` na każdym pliku. Komendy: `composer dump-autoload -o`, `php85 artisan config:clear`, `optimize:clear`. Smoke z serwera: `https://rsperformance.online/` i `/admin/login` → **200**. Rollback: `cp plik.bak_cursor_20260412 plik`.

[2026-04-12 ~22:00 CET] Cursor -> **VPS ai-gateway:** `/srv/ai-gateway/sync.sh` — w pętli pobierania `feeds/` dopisano **`ev-hybrid.json`** (wcześniej canonical miał feed, bramka `ai.rsperformance.online` go nie synchronizowała). Backup: `sync.sh.bak_cursor_ev_hybrid_feed_20260412`. `bash -n` OK, pełny sync OK; plik `/srv/ai-gateway/feeds/ev-hybrid.json` na miejscu; public `HEAD https://ai.rsperformance.online/feeds/ev-hybrid.json` → **200**, `Content-Length: 23066`. Repo: wpis w `SESSION_LOG.md` (brak mirroru `sync.sh` w git).

[2026-04-12 ~session CET] Cursor -> **repo (bez prod):** `scripts/run_n8n_fleet_verify_vps_auto.py` — `-h`/`--help` pokazuje docstring wrappera + pełny help `n8n_fleet_wow_verify_apr2026.py` **bez** `vps_exec`/JWT; `flush=True` na nagłówkach. `scripts/gravity_cursor_env.py` — `resolve_n8n_api()` (URL + klucz po `load_cursor_env`) używane przez fleet verify / suite / deep scan. Relay: `HANDOFF.md`, `SESSION_LOG.md`. Branch: `feature/v9-architecture-rebuild`.

[2026-04-12 ~session CET] Cursor -> **skill:** `.agents/skills/rs-n8n-wow-2026/SKILL.md` **v1.2.1** — dokumentacja wrappera `--help` (bez SSH) oraz `resolve_n8n_api()` w Primary sources; spójność z kodem fleet na `feature/v9-architecture-rebuild`.

[2026-04-21 CET] Cursor -> **VPS sync.sh — utrzymanie openapi.yaml:** Wgrano `/srv/ai-gateway/bin/mirror_openapi_yaml.py` (z `G:\gravity\scripts\vps_ai_gateway_mirror_openapi_yaml.py`), backup `sync.sh.bak_mirror_openapi_yaml_*`, idempotentny patch (`deploy/vps/patch_sync_openapi_yaml_mirror.sh` na `/tmp` + run) — przed `AI GATEWAY SYNC COMPLETE` dodane wywołanie mirroringu JSON→YAML; `bash -n` OK; ręczny run skryptu: backup YAML z timestampem + zapis 5333 B. Repo: skrypt + patcher + `RELAY.md` (checklist deploy hostingu z `CYBERFOLKS_SSH_PASSWORD`), `vps.md` (opis hooka).

[2026-04-21 CET] Cursor -> **VPS ai-gateway + repo (hosting↔VPS AEO):** Parity OpenAPI na bramce: `https://ai.rsperformance.online/.well-known/openapi.yaml` było **404** (kanon miał YAML). Backup na VPS: `openapi.json.bak_cursor_gateway_yaml_20260412`; upload wygenerowanego YAML (`vps_exec.py --upload` → `/srv/ai-gateway/.well-known/openapi.yaml`); smoke public **200**, `Content-Type: application/yaml`. Repo: `App\Support\RsUri::aiGatewayOpenApiYaml()`, `AiCitationHeaders` (RFC 8288 `Link` alternate YAML + `X-AI-Gateway-OpenAPI-YAML` dla katalogowanych botów), `SearchArtifactFactory` (`gateway.openapi_yaml` + wpis w `discoveryResources()`), `AiDiscoveryArtifactBuilder` (komentarz `# Gateway OpenAPI (YAML):` w `robots.txt`), test `tests/Unit/RsUriAiGatewayOpenApiYamlTest.php`, rozszerzenie `AiCitationHospitalityMiddlewareTest`. Docs: `RELAY.md` (operacyjna nota parity JSON/YAML na gateway), `.agents/skills/rs-aeo-skill/SKILL.md` (ostrzeżenie: przykładowy blok robots nie jest kopią produkcyjną), `robots_remote.txt` (nagłówek „SNAPSHOT — nie źródło prawdy”). **Deploy hosting:** po merge/SFTP zsynchronizować zmienione pliki PHP i zregenerować artefakty discovery (`SearchArtifactFactory::writeAll` / procedura deploy), żeby `ai-resources.json` i nagłówki HTML miały nowe URI — VPS YAML jest już live.

[2026-04-12 ~22:40 CET] Cursor -> **hosting live (AEO OpenAPI slice) + `ssh_exec` ergonomia:** Repo: `ssh_exec.py` — przed hasłem `load_cursor_env()` + fallback z pierwszej linii `cs.txt` (gitignored) lub `CYBERFOLKS_SSH_PASSWORD_FILE`; `.gitignore` (`cs.txt`); `RELAY.md` + `.cursor/mcp.env.example` (sekcja Cyber-Folks SSH). **Deploy SFTP** do `~/domains/rsperformance.online/laravel/`: backupi `*.bak_cursor_aeo_openapi_20260412` na `RsUri.php`, `AiCitationHeaders.php`, `AiDiscoveryArtifactBuilder.php`, `SearchArtifactFactory.php`; upload z repo; `php85 artisan search:artifacts-generate`; `php85 artisan optimize:clear`. Smoke z serwera: `/` **200**, nagłówek `Link` zawiera `openapi.yaml`; `ai-resources.json` zawiera `gateway.openapi_yaml` → `https://ai.rsperformance.online/.well-known/openapi.yaml`. **Uwaga:** plik `G:\gravity\cs.txt` nie był w workspace agenta — hasło działało z istniejącego `.cursor/mcp.env`; operator może trzymać hasło wyłącznie w `cs.txt` zamiast w `mcp.env`.

[2026-04-12 ~23:50 CET] Cursor -> **repo (operator WOW):** `scripts/smoke_aeo_gateway_openapi_canonical.py` — jeden strzał stdlib (`urllib`) na kanonie: `ai-resources.json` ma `gateway.openapi_yaml` kończące się na `/.well-known/openapi.yaml`, `HEAD /` ma `Link` z `openapi.yaml`. Zaktualizowano `RELAY.md` (wiersz deploy slice), `plan.md` (status), `HANDOFF.md` (hash commita **`eed7a34`** zamiast pośredniego `7c9e4d5` po amend). Lokalny run skryptu: exit **0**.

[2026-04-22 CET] Cursor -> **repo + hosting + VPS (MCP WOW / dyrektywy / smoke):** Git: commit **`5f67087`** na `feature/v9-architecture-rebuild` (push OK) — m.in. `.cursor/mcp.json` (postgres/qdrant/playwright), `RELAY`/`AGENTS`/`.cursor/rules/gravity-directives.mdc`, skrypty `tools/verify-cursor-mcp-wow.ps1`, `mcp-verify.cmd`, `qdrant-vps-mcp-runtime`, `package.json`, skille `rs-a2a-wow-2026`, krzyżówki `rs-n8n`/`rs-discovery`. **Hosting:** backup `AiDiscoveryArtifactBuilder.php.bak_cursor_mcp_20260422`, SFTP upload `app/Support/Search/AiDiscoveryArtifactBuilder.php`, `php85 artisan search:artifacts-generate` + `optimize:clear` (OK). **VPS:** `composer a2a:sync-gateway` (exit 0). **Smoke (HEAD/GET):** `rsperformance.online/` + `/.well-known/a2a.json` + `ai-resources.json` + `robots.txt`, `ai.rsperformance.online/.well-known/answer-routing.json`, `https://auto.rs3d.pl/healthz` — wszystkie **200**. Rollback hosting: przywróć z `.bak_cursor_mcp_20260422`.

[2026-04-22 CET] Cursor -> **bezpieczne wdrożenie kontynuacji (dok + MCP + bramka, bez downgrade):** Zgodnie z `gravity-directives.mdc` / `AGENTS.md` — **brak losowych MCP z GitHub** (pinowany stos `.cursor/mcp.json`); `npm run mcp:install` + `npm run mcp-verify` + `composer mcp-verify` → **Tier S** (uv + Qdrant `.venv`, runtimy WOW). **Hosting (Cyber-Folks):** katalog Laravel **nie jest repozytorium git** (`git rev-parse` → brak) — ta partia **nie wymagała** nowego SFTP (tylko `docs/ops` + `tools/` w repo). **VPS:** `composer a2a:sync-gateway` → `sudo /srv/ai-gateway/sync.sh` (exit **0**). **GitHub:** push `7f42f54` na `origin/feature/v9-architecture-rebuild` — m.in. poprawka tabeli §8 w `HOSTING-VPS-AUDIT-2026-04-22.md` (PHP 8.5.3 vs Laravel 13.1.1, format). **Smoke HTTP:** `https://rsperformance.online/` + `/.well-known/a2a.json`, `https://ai.rsperformance.online/.well-known/openapi.json` → **200**. Następny deploy PHP na hosting: **SFTP** wybranych plików + backup `*.bak_*` + `artisan` wg `RELAY` — nie `git pull`.

[2026-04-22 ~20:10 CET] Cursor -> **hosting live: A2A ListTasks + karta 2.2.1 (Overture):** Wdrożono **`SearchArtifactFactory.php`** (backup `SearchArtifactFactory.php.bak_cursor_a2a_card_212_20260422`) + regeneracja `search:artifacts-generate` + `optimize:clear` — karta deklaruje `GET /tasks` oraz `ListTasks`/`tasks/list` w `supportedInterfaces`. Pierwsza próba Overture: **`list-tasks` skip** (karta OK, brak metody) — sonda JSON-RPC: `-32601 Method not found: ListTasks`. Wdrożono **`A2aTaskController.php`** (backup `A2aTaskController.php.bak_cursor_listtasks_20260422`); `php85 -l` OK; `optimize:clear`. **Overture certify** (`npx overture certify https://rsperformance.online --json`): **18 passed, 0 failed, 0 warnings, 6 skipped**; **`list-tasks` pass** ("4 task(s) returned"). **VPS:** `composer a2a:sync-gateway` po pliku fabryki (przed kontrolerem) — OK. **MCP a produkcja:** serwery z `.cursor/mcp.json` to operator (Cursor) + tunele; Qdrant/n8n/Postgres na VPS już w architekturze — **brak** instalacji losowych MCP na VPS poza obecnym stosem w `vps.md` / `handoff` (n8n-mcp kontener itd.). **Lokalne:** `SSLKEYLOGFILE` może psować `urllib` — ustaw pusty przed sondami HTTPS.

[2026-04-22 CET] Cursor -> **hosting + VPS + repo: A2A skill WOW `ev-hybrid-knowledge` (karta 2.2.2):** Zgodnie z `rs-a2a-wow-2026` / `AGENTS.md` / `RELAY.md` — **brak** losowych MCP z GitHuba; deploy = SFTP + artefakty. **Git:** `be4060d` na `origin/feature/v9-architecture-rebuild` (A2A: pełny wpis `ev-hybrid-knowledge` w publicznej karcie, testy, skill **1.2.11**). **Hosting:** `ssh_exec.py --upload` → `app/Support/Search/SearchArtifactFactory.php`; `php85 artisan search:artifacts-generate` + `optimize:clear` — wygenerowano m.in. `/.well-known/a2a.json` + `agent-card.json`. **Smoke:** `GET https://rsperformance.online/.well-known/agent-card.json` → `version` **2.2.2**, `id` **ev-hybrid-knowledge** obecne. **VPS:** `python scripts/vps_ai_gateway_sync_apr2026.py` (exit **0**). Następny check (opcjonalny): `php composer.phar a2a:certify` lokalnie lub `npm run a2a:certify` — regres Overture po zmianie karty.

[2026-04-22 ~22:00 CET] Cursor -> **hosting live + VPS: A2A 2.2.3 (WOW) + JSON-RPC copy:** Backupy: `SearchArtifactFactory.php.bak_cursor_a2a_223_20260422`, `A2aTaskController.php.bak_cursor_a2a_223_20260422` w `domains/rsperformance.online/laravel/`. SFTP `ssh_exec.py --upload` z `G:\gravity`: `app/Support/Search/SearchArtifactFactory.php`, `app/Http/Controllers/A2aTaskController.php`. Komendy: `php85 artisan search:artifacts-generate --no-interaction`, `php85 artisan optimize:clear --no-interaction`. **VPS:** `python scripts/vps_ai_gateway_sync_apr2026.py` (exit **0**). **Weryfikacja public:** `GET https://rsperformance.online/.well-known/agent-card.json` → `version` **2.2.3**, `x_rs_a2a_operator_2026_04` obecny. Dyrektywa: `gravity-directives.mdc` — agent **nie pyta** o deploy na prod (wykonuje wg RELAY). Repo: commit kontrolny po tej operacji (skill **1.2.12**, test, fabryka, dyrektywa).

[2026-04-26 CET] Cursor -> **VPS n8n (`https://auto.rs3d.pl`):** wdrożono **Telegram WOW suite** — `python scripts/run_n8n_telegram_wow_suite_vps_auto.py --json` (JWT z `user_api_keys` na VPS). PUT OK: Research Harvester `xcwu34W87JpmV75S`, DTC Enrichment `9oAbPosvf0h860Xg`, DTC IndexNow Drip `v2p1MYtzCVmxUxyU`; lokalne backupy `scripts/.tmp_n8n_wf_backup_20260426_*`. Dopis: `SESSION_LOG.md` (blok sesji).

[2026-04-26 CET] Cursor -> **VPS n8n (tylko odczyt, produkcja):** audyt **duplikatów nazw** workflowów via API — **24** rekordy, **22** unikalne nazwy; duplikaty nazw: `RS AI Bot Invitation Hub` (aktywny `F6uosr6xSCJZM4fO` + nieaktywny `FM1BxBIDKRmhr57i`), `RS Blog on Demand (Telegram)` (aktywny `BiCeE7CqPPfVsad0` + nieaktywny `VPTe4OfoyBuGgddS`). Uzupełnienie `SESSION_LOG.md` (blok audytu) + `AGENTS.md` (nakaz: weryfikacja prod = wpis w repo).

[2026-04-26 CET] **Reguła właściciela (przypomnienie w repo):** praca **wyłącznie w kontekście produkcji** (hosting, VPS, live n8n); **zapis w repozytorium** tego, co wykonano (`handoff-log.md`, `SESSION_LOG.md`, ewent. `start.md` wg `AGENTS.md`) — **nakaz**, nie prośba; dotyczy też operacji **tylko odczytowych** na prod.

[2026-04-26 CET] Cursor -> **VPS n8n + Telegram (produkcja `https://auto.rs3d.pl`, odczyt):** `python scripts/run_n8n_fleet_verify_vps_auto.py --json` oraz `--definition-gate --json`. **24** workflowy. Telegram: `getMe` **200**, `@rsperformance_bot`, **6534705697: OK** vs env. **2** aktywne z `last=error` (404 „resource not found”): `yE7tLieYNJa4FDW3` SEO-AEO, `F6uosr6xSCJZM4fO` Invitation Hub — przy `--definition-gate` oznaczone jako **stale** (`ok: true`, `flagged_count: 0`). Research/DTC/IndexNow WOW: **last=success**. Dopis: `SESSION_LOG.md` (blok weryfikacji).

[2026-04-26 CET] Cursor -> **VPS production update + rollback-ready backup:** backup `/srv/backups/vps/update_20260426_100049_cursor` (APT/Docker manifests, `/srv/ops-stack/compose`, n8n SQLite, Qdrant snapshots, Caddy/systemd/ai-gateway configs). APT upgrade: Docker **29.4.1**, Docker Compose **5.1.3**, containerd **2.2.3**, Google Cloud SDK **565.0.0**, CrowdSec **1.7.7**, systemd **255.4-1ubuntu8.15**; controlled reboot -> kernel **6.8.0-110-generic**, no reboot-required. Docker images refreshed: n8n **2.17.7**, n8n-mcp latest, Qdrant latest, Postgres 17/15-alpine, Umami latest, SearxNG latest; `rs-support-plane-app` kept local. Fix: n8n Python task runner warning resolved by external sidecar `n8nio/runners:2.17.7`, broker local-only `127.0.0.1:5679`, n8n pinned to `n8nio/n8n:2.17.7`. Smoke OK: n8n/n8n-mcp/Qdrant/ai-gateway/analytics/auto health; fleet `--definition-gate --json` -> **ok true**. Repo: `start.md`, `SESSION_LOG.md`, `infrastructure/vps/docker/docker-compose.yml`.

[2026-04-26 CET] Cursor -> **VPS cleanup/final smoke:** po aktualizacjach wykonano `apt-get clean` + `docker image prune -f` tylko dla nieużywanych obrazów; odzyskano ok. **1.346 GB**, `/` finalnie **72% used / 26G free**. Final smoke: kernel `6.8.0-110-generic`, `apt_upgradable=0`, `https://auto.rs3d.pl/healthz` **200**, AI gateway **200**, analytics **200**, lokalnie n8n/broker/n8n-mcp/Qdrant **200**.

[2026-04-26 10:49 CET] Cursor -> **VPS n8n + Telegram WOW final audit/fix (produkcja `https://auto.rs3d.pl`):** użyto skill `rs-n8n-wow-2026` + n8n validation/MCP guidance. Fleet `run_n8n_fleet_verify_vps_auto.py --definition-gate --json` -> **ok true**, finalnie `flagged_count=0`, **24 workflowy**, **20 aktywnych**; `RS AI Bot Invitation Hub` po patchu ma ostatni run **success** (`2026-04-26T08:48:10Z`). Telegram Bot API `getMe` -> **200**, bot `@rsperformance_bot`, chat `6534705697` spójny; realny UTF-8 smoke przez Bot API -> **HTTP 200**, `echo_matches=true` dla `Zażółć gęślą jaźń / Łódź / Śląsk / źdźbło`. Naprawiono VPS helpery `/srv/ops-stack/scripts/vps_n8n_telegram_wow_digest.py` + dodano `vps_n8n_wow_smoke.py` (backup `vps_n8n_telegram_wow_digest.py.bak_cursor_digest_parser_20260426`): parser czyta `chat_id` z poprzedniego Code node po migracji `jsonBody={{ JSON.stringify($json) }}` i wysyła `Content-Type: application/json; charset=utf-8`. Digest finalnie: **Telegram HTTP 200**, health fails **0**, invitation webhook **200**. Runtime fixes przez n8n API po backupach SQLite (`database.sqlite.bak_cursor_n8n_runtime_fix_20260426`, `database.sqlite.bak_cursor_n8n_seo_aeo_kontakt_fix_20260426`): `F6uosr6xSCJZM4fO` Telegram URL -> `RS_TELEGRAM_BOT_TOKEN`/`RS_TELEGRAM_CHAT_ID`; `yE7tLieYNJa4FDW3` SEO-AEO usunięto martwe URL-e `/diagnostyka` i `/kontakt`, wszystkie **11** targetów SEO-AEO -> HTTP **200**. Residual uczciwy: historia n8n nadal pokazuje stale last error dla SEO-AEO do następnego naturalnego schedule; Public API `/execute` zwraca **405**, CLI `n8n execute` koliduje z aktywnym brokerem `5679`, więc nie wymuszano ryzykownego runu.

[2026-04-26 10:58 CET] Cursor -> **Produkcja (read-only) + pakiet wiedzy `g:/gravity/wiedza`:** zweryfikowano live profil RS z `rsperformance.online` (`/`, `/uslugi`, `/kody-usterek`, `llms.txt`, `.well-known/ai-resources.json`) i zebrano wartościowe źródła zewnętrzne 2026+ pod użytkowników + AI (DTC/VIN/UDS/knowledge graph): `Wal33D/dtc-database`, `NHTSA vPIC API`, `NHTSA datasets`, `mdabrowski/uds`, `tbohne/obd_ontology`, `OBDb`. Utworzono katalog `G:\gravity\wiedza` i zapisano pakiet: `README.md`, `01_production_profile.md`, `02_sources_high_value_catalog.md`, `03_ingestion_blueprint.md`, `04_publish_plan_for_users_and_ai.md`, `sources_manifest.json`.

[2026-04-26 11:05 CET] Cursor -> **Rozszerzenie pakietu wiedzy o pełen zakres usług RS (internet):** dodano usługowe targety wiedzy dla: wulkanizacja/opony, geometria, klimatyzacja (R134a/R1234yf + F-gas), zawieszenie, sprzęgła, skrzynie, rozrządy, przeglądy/oleje, hamulce, oświetlenie. Źródła wysokiej wartości: UNECE R30/R13-H/R48, SERMI + CELEX 32021R1244, EU F-gas portal, ZF Aftermarket, Schaeffler LuK, Gates Tech Tips, Hunter alignment + wcześniejsze DTC/VIN/UDS/KG. Zapisano: `G:\gravity\wiedza\05_service_knowledge_targets.md` i zaktualizowano `G:\gravity\wiedza\sources_manifest.json` + `README.md`.
[2026-04-26 11:10 CET] Cursor -> **Automatyczny downloader wiedzy + snapshot (produkcja knowledge plane):** dodano `scripts/download_wiedza_sources_apr2026.py` (zbiera URL-e z `wiedza/sources_manifest.json` + `wiedza/05_service_knowledge_targets.md`, pobiera surowe źródła do `wiedza/raw/<timestamp>/`, generuje `wiedza/knowledge_snapshot.json` i `wiedza/knowledge_for_ai.json`; fallbacki dla części źródeł blokowanych). Wykonano run: `python scripts/download_wiedza_sources_apr2026.py` -> **37 URL**, **28 OK**, **9 failed** (głównie 403/500 anti-bot po stronie zewnętrznych serwisów: Schaeffler/ZF/Hunter/GarageWire + brak README dla `mdabrowski/uds` raw branch). Zaktualizowano `wiedza/README.md` o instrukcję cyklicznego snapshotu i artefakty feedu AI.
[2026-04-26 12:12 CET] Cursor -> **Próg jakości 80% dla snapshotu wiedzy (repo + VPS):** w `scripts/vps_wiedza_snapshot_runner.py` i `scripts/download_wiedza_sources_apr2026.py` dodano bramkę: kanoniczne `knowledge_snapshot.json` / `knowledge_for_ai.json` aktualizują się tylko gdy `(OK+ok_fallback)/total ≥ min` (domyślnie **80%**, `--min-ok-rate` / env `RS_WIEDZA_MIN_OK_RATE`). Poniżej progu: surowe pliki zostają w `raw/<ts>/`, zapis `knowledge_*_rejected.json`, Telegram z komunikatem odrzucenia (gdy włączony), exit **1**; poprzedni kanon nie jest nadpisywany. Wdrożono runner na VPS (backup `vps_wiedza_snapshot_runner.py.bak_cursor_wiedza_quality_gate_20260426`); smoke: **83.8%** vs próg 80% → **pass**; test odrzucenia `--min-ok-rate 0.95 --no-telegram` → **reject** + pliki `*_rejected.json`. Zaktualizowano `wiedza/README.md`, `.cursor/mcp.env.example` (`RS_WIEDZA_MIN_OK_RATE`).

[2026-04-26 11:59 CET] Cursor -> **VPS cykliczny snapshot wiedzy + Telegram digest Gemini (cheap):** wdrożono na produkcji `/srv/ops-stack` job wiedzy: upload `scripts/vps_wiedza_snapshot_runner.py`, `scripts/vps_install_wiedza_snapshot_cron.sh`, `wiedza/sources_manifest.json`, `wiedza/05_service_knowledge_targets.md` do `knowledge-sync`; backupy przed podmianą (`*.bak_cursor_wiedza_*`, `crontab.bak_before_wiedza_snapshot_*`). Cron zainstalowany idempotentnie: **co 6h o :15 Europe/Warsaw** (`/srv/ops-stack/logs/wiedza_snapshot.log`). Runner generuje: `/srv/ops-stack/knowledge-sync/raw/<ts>/`, `knowledge_snapshot.json`, `knowledge_for_ai.json`, oraz wysyła raport Telegram. Warstwa Gemini ustawiona na **tani model `gemini-2.5-flash-lite`** (preferencja OpenRouter `OPENROUTER_API_KEY` z `/srv/ops-stack/knowledge-sync/.env`; fallbacky Google API/Vertex). Smoke po deploy: `total=37`, `ok=31`, `failed=6`, Telegram **sent** z digestem Gemini.

[2026-04-26 12:26 CET] Cursor -> **Read-only audit: hosting MySQL ↔ VPS/Qdrant real support-plane:** potwierdzono, że strona publiczna zostaje na hostingu (`APP_URL=https://rsperformance.online`, DB `mysql`), a hosting ma ustawiony `services.diagnosta.url=https://mcp.rs3d.pl` i token (`diagnosta_token_set=true`). Z hostingu wykonano bezpieczny `diagnostic-search` do VPS -> HTTP **200**, `total=3`, pierwsze trafienie **Diagnostyka komputerowa**. VPS: `diagnosta-api.service`, `diagnosta-mcp.service`, `caddy` aktywne; Docker: Qdrant/Postgres/n8n/n8n-mcp up. Qdrant: `rs_static_knowledge` **2574** punktów, `rs_dynamic_knowledge` **159**, `rs_answer_routing` **39**; Postgres `documents` **2572** (`static=2413`, `dynamic=158`, `blog_post=1`). Diagnosta OpenAPI wystawia `/internal/content-sync` i `/internal/content-feed-pull`; hostingowe publiczne `/api/content-feed` zwraca **404** — real-time observer sync istnieje, ale fallback pull feed wymaga domknięcia.

> LIVE KNOWLEDGE SEO INDEX LAYER (2026-04-26 13:19 CET)
> Hosting `rsperformance.online`: wdro?ono indeksowaln? warstw? HTML nad Qdrant: `/wiedza-ai` + 8 temat?w `/wiedza-ai/{slug}` (m.in. `p0420-katalizator-sonda-lambda`, `p0299-brak-doladowania-turbo`, DPF/EGR, AdBlue, klimatyzacja, skrzynia). Backupy przed deployem: `routes/web.php.bak_cursor_kseo_*`, `app/Support/RsUri.php.bak_cursor_kseo_*`, `app/Support/Search/SearchArtifactFactory.php.bak_cursor_kseo_*`. Nowe pliki: `KnowledgeSeoController`, `KnowledgeSeoTopicCatalog`, widoki `resources/views/pages/knowledge-seo/*`, test `KnowledgeSeoPagesTest`.
> Cel: dane Qdrant s? teraz dost?pne nie tylko przez API JSON, ale jako canonical HTML dla SEO/GSC: answer-first text, Qdrant evidence cards, canonical, `index, follow`, schema `CollectionPage`/`TechArticle`/`FAQPage`/`Dataset`, wpisy w `sitemap.xml`, `llms-full.txt`, `ai-resources.json`.
> Smoke live: `/wiedza-ai` 200, `/wiedza-ai/p0420-katalizator-sonda-lambda` 200, `sitemap.xml` zawiera `/wiedza-ai/p0420-katalizator-sonda-lambda`, `llms-full.txt` i `ai-resources.json` zawieraj? nowe URL-e, `/api/knowledge/search?q=P0420+katalizator&limit=1` zwraca `source=qdrant`. Produkcyjny test `php85 artisan test --compact tests/Feature/KnowledgeSeoPagesTest.php` -> 3 passed / 13 assertions. IndexNow flush -> `last_result=success`, `pending_count=0`, `submitted_count=12`.

[2026-04-26 13:39 CET] Cursor -> **hosting live: Local Gdańsk district SEO/AEO/GEO layer** — wdrożono indeksowalne strony dzielnicowe: `/gdansk` + `/gdansk/{slug}` dla `wrzeszcz`, `oliwa`, `przymorze`, `zaspa`, `morena`, `chelm`, `osowa`, `siedlce`, `brzezno`, `jasien`. Pliki: `app/Support/LocalSeo/LocalAreaCatalog.php`, `app/Http/Controllers/LocalAreaSeoController.php`, `resources/views/pages/local-seo/{index,show}.blade.php`, `tests/Feature/LocalSeoPagesTest.php`; zmiany w `routes/web.php`, `App\Support\RsUri`, `SearchArtifactFactory`. Backup prod: `storage/app/backups/local-seo-20260426_133712`. Artefakty: `php artisan search:artifacts-generate` zapisał `sitemap.xml`, `llms.txt`, `llms-full.txt`, `.well-known/ai-resources.json`. Testy: lokalnie `LocalSeoPagesTest` 3 passed / 20 assertions i `KnowledgeSeoPagesTest` 3 passed / 13 assertions; produkcja `LocalSeoPagesTest` 3 passed / 20 assertions. Smoke publiczny: `/gdansk`, `/gdansk/wrzeszcz`, `/gdansk/oliwa`, `sitemap.xml`, `llms-full.txt`, `ai-resources.json` -> **200** + brak brakujących markerów; pełny smoke 10 dzielnic -> **10/10 HTTP 200**, schema `AutoRepair` + `FAQPage` + link `/api/knowledge/search` obecne. IndexNow: `last_result=success`, `last_status_code=200`, `url_count=11` nowych URL-i dzielnicowych.

[2026-04-26 13:56 CET] Cursor -> **hosting live: Service SEO/AEO/GEO WOW graph dla wszystkich usług** — wzmocniono `/uslugi` i `/uslugi/{slug}` tak, żeby cały katalog usług był promowany jako answer-first service graph: hub ma sekcję `Answer-first service graph`, metryki katalogu, linki do `/problemy`, `/gdansk`, `/wiedza-ai`, pełną mapę kategorii usług i lokalne linki dzielnicowe; strony usług mają `AI-readable service hook` z publicznym `/api/knowledge/search?q=...` oraz sekcję dzielnic Gdańska. `SearchArtifactFactory::orderedServices()` obejmuje teraz wszystkie aktywne usługi (nie tylko ręczną listę priorytetową), a `ai-resources.json` wystawia pełne `service_atomic_answers` dla 19 usług. Backup prod: `storage/app/backups/service-seo-wow-20260426_1345`. Wdrożone pliki: `app/Http/Controllers/ServiceController.php`, `resources/views/pages/services-index.blade.php`, `resources/views/pages/service-show.blade.php`, `app/Support/Search/SearchArtifactFactory.php`, `tests/Feature/ServiceSeoGraphTest.php`. Weryfikacja: lokalnie `ServiceSeoGraphTest` 3 passed / 12 assertions; produkcja `php85 -l` OK, `ServiceSeoGraphTest` 3 passed / 12 assertions, `optimize:clear`, `search:artifacts-generate`. Public smoke: `/uslugi`, `/uslugi/diagnostyka-komputerowa`, `/sitemap.xml`, `/llms-full.txt`, `/.well-known/ai-resources.json` -> **200**, `ai-resources.json services=19`, brak `kalibracja-adas`. Uwaga jakościowa: pierwszy wariant testu stworzył tymczasowy rekord `kalibracja-adas`; został wykryty, usunięty z DB (`count=0`), stale `.md` usunięte, test przerobiony na niemutujący i artefakty wygenerowane ponownie. IndexNow flush: `url_count=24`, `last_result=success`, `status_code=200`, `pending_count=0`.

[2026-04-26 15:10 CET] Cursor -> **hosting + VPS n8n live: editorial image gate dla wszystkich blog/message workflows** — audyt `https://auto.rs3d.pl` wykazał główne workflowy: `RS Daily Automotive News Drafts`, `RS Blog on Demand (Telegram)`, `RS AI Agent Content`, `RS Editorial Board`, plus DTC/research/knowledge flows. Hosting: backup trzech plików (`*.bak_20260426_n8n_image_gate`), SFTP `BlogVertexPipelineService.php`, `BlogPipelineController.php`, `config/blog.php`, `php85 -l` OK, `php85 artisan optimize:clear`, `config:show blog.require_hero_image_for_pipeline` -> **true**. Backend: `/api/blog/pipeline/run` przyjmuje `require_hero_image`; quality-enforced pipeline rzuca `HERO_IMAGE_REQUIRED`, gdy tekst przejdzie, ale obraz nie powstanie; `/api/blog/pipeline/persist` robi centralny SEO/AEO/GEO pass i dopina hero image przed zapisem przygotowanego draftu. VPS: backup SQLite `database.sqlite.bak_n8n_image_gate_20260426`; n8n API patch przez `scripts/n8n_apply_editorial_image_gate_apr2026.py` z lokalnymi backupami workflow JSON. Patche: `RS Daily Automotive News Drafts`, `RS Blog on Demand`, `RS AI Agent Content` mają `require_hero_image: true`; `RS Editorial Board` przekazuje `source_urls`, slot/date, research brief i konkretny `featured_image_alt` do centralnego pipeline. Weryfikacja: `run_n8n_fleet_verify_vps_auto.py --definition-gate --json` -> **ok true**, `flagged_count=0`; audyt definicji potwierdził image/hero sygnały i pipeline routes; lokalne testy `BlogPipelineImageGateTest`, `N8nVertexProxyTest`, `BlogTelegramWebhookTest` -> **10 passed / 14 assertions**; produkcja `BlogPipelineImageGateTest` -> **1 passed**; `/api/blog/pipeline/*` route-list OK. Residual uczciwy: `RS AI Agent SEO-AEO` nadal ma historyczny `DEFOK` do następnego naturalnego sukcesu, bez nowego błędu definicji.

[2026-04-26 15:47 CET] Cursor -> **hosting + VPS n8n live: Search Ops GSC Harvester/Promoter dla 19 usług** — dopracowano `RS Search Ops Harvester` (`vRaEJ2FjiT0LIOr0`) i `RS Search Ops Promoter` (`SY44YRLCeJlivF2T`). Hosting: backup `storage/app/backups/search_ops_20260426_1339`; SFTP `config/search_ops.php`, `OpportunityRouter.php`, `OpportunityIntentClassifier.php`, `OpportunitySnapshotStore.php`, `MicroExpansionDraftService.php`, `SupportPageDraftService.php`, `SearchOpsController.php` + testy. `service_route_map` obejmuje 19 usług; DTC-like queries (`P/B/C/Uxxxx`) wzmacniają `/uslugi/diagnostyka-komputerowa`; snapshoty GSC+Bing są merge per source, nie overwrite. n8n: nowy skrypt `scripts/n8n_apply_search_ops_wow_apr2026.py`, PUT OK obu workflowów, `Promoter` aktywowany; lokalne exporty `n8n_workflow_search_ops_*.json` zsynchronizowane. Weryfikacja: lokalnie Search Ops testy **26 passed**; produkcja `php85 artisan test ...SearchOps...` -> **27 passed / 69 assertions**; API smoke `fetch-signals` **200 ok**, sample promote `klimatyzacja nie chlodzi gdansk` -> **ready_for_apply** `/uslugi/klimatyzacja-serwis`; webhooki `search-ops-harvest-manual` i `search-ops-promote-manual` -> **200**, Telegram sent, Harvester `OK gsc` + `OK bing`, Promoter `Ready: 12`, `Review: 12`; fleet `run_n8n_fleet_verify_vps_auto.py --definition-gate --json` -> **ok true**, `flagged_count=0`, oba Search Ops workflowy active + last `success`.

[2026-04-26 17:25 CET] Claude -> **hosting + VPS: punkty 1-3 zamkniete (Vertex deprecje / n8n dedup / A2A 0.3.0)** — Punkt 1: Vertex JWT auth false-alarm (kod uzywa poprawnego `urn:ietf:params:oauth:grant-type:jwt-bearer`; blad pochodzil z php -r ad-hoc). Naprawione realne: `app/Services/GoogleTokenService.php` - hardcoded path -> multi-candidate resolver (env + ~/google-credentials.json + legacy), usuniete deprecated `curl_close` (PHP 8.5). `app/Support/SearchOps/SearchConsoleService.php:376` + `SearchPerformanceService.php:127` - usuniete `openssl_free_key` (deprecated PHP 8.0, removed 8.5). Backupy `.bak_claude_path_fix_20260426`, `.bak_claude_openssl_dep_20260426`. Verify: Vertex token=512, GoogleToken=1024, fetchSignals(2,1) status=ok bez warnings. Punkt 2: n8n duplicate workflows zarchiwizowane via REST `POST /api/v1/workflows/{id}/archive` (n8n >=1.69) + SQLite UPDATE name. `FM1BxBIDKRmhr57i` (Bot Invitation Hub dup), `VPTe4OfoyBuGgddS` (Blog on Demand dup), `KU0A2HAd4WUErx8h` (Smart AI Inviter superseded) - wszystkie [ARCHIVED 2026-04-26 ...]. Canonical `F6uosr6xSCJZM4fO` + `BiCeE7CqPPfVsad0` nietkniete. Bonus: `/srv/ops-stack/n8n/storage/` bylo 100% pelne, usuniete 7 backupow ~22 GB; `bak_n8n_image_gate_20260426` zachowany. Punkt 3: A2A 0.3.0 verified vs canonical proto github.com/a2aproject/A2A. Pole nazywa sie `protocolBinding` (NIE `transport`) - open-form string per spec. AgentCard.protocolVersion=0.3.0, wszystkie required fields obecne (name/description/supportedInterfaces/version/capabilities/defaultInputModes/defaultOutputModes/skills x6). Bug fix: JSONRPC interface mial protocolVersion="2.0" (to wersja JSON-RPC) -> poprawione na "1.0" (per proto: A2A wire version, examples 0.3/1.0). Backup `agent-card.json.bak_claude_protover_20260426`. Verify: 4 interfaces wszystkie protocolVersion=1.0. Carryover: backup-rotation policy /srv/ops-stack/n8n/storage/, env tuning EXECUTIONS_DATA_PRUNE_MAX_COUNT=2000 + EXECUTIONS_DATA_MAX_AGE=168, BING_WEBMASTER_API_KEY -> AiInvitationService cron, Heniu Diagnosta OpenAPI +EV (Gap 4). Drobne extras na agent-card (capabilities.stateTransitionHistory, top-level authentication/url) tolerowane bo open extension, non-breaking.

[2026-04-26 ~18:00 CET] Cursor -> **hosting live: GSC status sync API (WOW / dyrektywy):** Wdrożono **POST `api/ops/gsc-status/sync`** (`SearchOpsController@syncGscStatus`) — tożsame z `php artisan search-ops:gsc-fetch` (zapis `search_ops.status_path`, N8nInternalAuth, throttle 30/min). Pliki: `SearchOpsController.php`, `routes/api.php`, `tests/Feature/Search/GscStatusSyncEndpointTest.php`. Backupy: `SearchOpsController.php.bak_cursor_gscsync_20260426`, `routes/api.php.bak_cursor_gscsync_20260426`, `SearchConsoleService.php.bak_cursor_gscsync_20260426` (stary miał `final` — blokował Mockery; zsynchronizowano z repo: klasa bez `final`, `Arr::first` w mapowaniu wierszy GSC). SFTP + `composer dump-autoload -o`, `optimize:clear`. Weryfikacja prod: `route:list` → **POST api/ops/gsc-status/sync**; `php85 artisan test tests/Feature/Search/GscStatusSyncEndpointTest.php` → **3 passed**; `search-ops:gsc-fetch` → **OK** (żywe API GSC). Dyrektywa `gravity-directives.mdc` §0a: brak losowych MCP z GitHub — pin w `.cursor/mcp.json`; repozytorium: commit 4 plików po deployu.
