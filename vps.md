...
- **Postgres (Docker na VPS, `compose`)**: wspólna instancja dla m.in. n8n / umami / rs_knowledge (patrz `infrastructure/vps/docker/docker-compose.yml`, `HANDOFF` — `127.0.0.1:5432` z hosta VPS). Z **laptopa** do MCP w Cursorze: **tylko tunel SSH** (np. `-L 15432:127.0.0.1:5432`), potem `DATABASE_URL` w `.cursor/mcp.env` i `POSTGRES_MCP_ENABLED=1` — wpis `postgres-vps-wow` w `G:\gravity\.cursor\mcp.json`, launcher `tools/postgres-vps-mcp-runtime`. Bez wystawiania Postgresa publicznie.
