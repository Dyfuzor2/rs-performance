...
- Current status: [-W TRAKCIE-] **2026-04-12 (repo):** `run_n8n_fleet_verify_vps_auto.py --help` bez SSH + `gravity_cursor_env.resolve_n8n_api()` dla skryptów fleet (`77abe85` na `feature/v9-architecture-rebuild`); exact lookup metadata is live, DTC public routes are restored, **live hosting now registers `AiCitationHeaders` in `bootstrap/app.php` so `/kody-usterek` surfaces always emit `X-Citation-Policy` and `X-Preferred-Citation` (including generic fetcher UAs)**, **2026-04-12: Filament „n8n WOW Ops Hub” wdrożony na hostingu** (`/admin/n8n-workflow-documents`, migracja `n8n_workflow_documents`, seed katalogu), **2026-04-12 wieczór: `public_html/.htaccess` zsynchronizowany z `config/ai_agents.php`** (ModSec 99000/99001 + WOW gateway — m.in. `Exabot`, `OpenRouter`, Cohere/HF/Reddit/Shap/ScaleAI/Together; backup `.htaccess.bak_cursor_aitokens_20260412`; smoke katalogu agentów **0×406** na zaproszonych), **2026-04-12 późno: hosting canonical — slice AEO OpenAPI YAML** (deploy PHP `RsUri` / `AiCitationHeaders` / `AiDiscoveryArtifactBuilder` / `SearchArtifactFactory`; `search:artifacts-generate`; `gateway.openapi_yaml` live w `ai-resources.json`; repo `ssh_exec.py` + `cs.txt` gitignore + `scripts/smoke_aeo_gateway_openapi_canonical.py`; commit `eed7a34` na `feature/v9-architecture-rebuild`), VPS exact-first resolver returns canonical DTC/service/problem winners for exact intents, the invitation plane now exposes a real standards-first hospitality fast lane (`X-AI-Hospitality`, preferred fetch order, `for-agents`, `for-ai-browsers`) instead of only silent manifests, the weak-boost / underboost lane now resolves to the canonical problem page across multiple human symptom phrasings, clean telemetry now shows real homepage / priority-path shares instead of false zeroes, canonical discovery now exposes a homepage-exit map plus agent-specific first-hop hints for the homepage-heavy AI families, the homepage still emits exact answer-path hints in response headers, canonical `home.md` mirrors that same first-hop contract for markdown-consuming agents, the public UI no longer shows the technical answer-rail block, the exact Anthropic crawler identities blocked by shared-host `406` are now narrowly rescued to VPS again, the blog system now has a same-day `daily_news` quality-gated workflow on VPS `n8n` (`preview -> quality gate -> persist`) with Telegram operator reporting, hosting telemetry again writes to `ai_bot_visits` while shadow-capturing non-catalog AI-like user agents such as `DeepSeek-Unknown`, and the daily-news lane is now materially harder against junk because both Laravel and `n8n` enforce anti-bait editorial rules plus slot-specific `porada/news/premiera` routing before any draft can persist.
- Current blocker: **VPS `n8n`** — **2026-04-12 wieczór:** flota HttpRequest `jsonBody` znormalizowana do `={{ JSON.stringify(...) }}` (Monitor + Code `Prepare Diagnosta Body`, Trinity, Trending, Content, SEO-AEO, Blog on Demand); **ostatnie exec w API nadal pokazują stare `error`** do czasu **następnego harmonogramu** — potrzebna obserwacja zielonego runu. Nadal do triage: `RS Daily Automotive News Drafts`, `RS Editorial Board` jeśli błąd nie był tylko HttpRequest.
- Current next step: **operator:** wizualny smoke Filament **n8n WOW Ops Hub** po zalogowaniu + ewentualnie sync z API (`.env` `N8N_API_URL` / `N8N_API_KEY`); po kolejnym cyklu cron: `python scripts/n8n_fleet_health_apr2026.py` + ewentualnie `GET .../executions?...&includeData=true` dla workflowów wciąż `error`; WOW digest VPS 08:00 jak w `rs-n8n-wow-2026`; opcjonalnie smoke `https://rsperformance.online/api/n8n/vertex/*`.
...
### 2026-04-12 VPS n8n Vertex import addendum

- **Done:** `n8n import:workflow` w kontenerze `n8n` (v2.14.2) z `/home/node/.n8n/import_vertex_patched.json` (źródło repo: `storage/app/n8n_vertex_patched.json`); backup przed: `/srv/ops-stack/n8n/storage/backup_pre_vertex_import_20260412.json`.
- **Done (wow):** `n8n publish:workflow --id=…` dla 16 workflowów, **bez** drugiego duplikatu `RS AI Bot Invitation Hub` (`FM1Bx…`), potem `docker compose restart n8n` w `/srv/ops-stack/compose`.
