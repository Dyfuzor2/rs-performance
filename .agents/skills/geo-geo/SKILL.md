...
> **RS Performance workspace:** Ten skill (`geo-geo`) jest **duplikatem zestawu** modułów `geo-*`. Kanoniczna mapa i kolejność pracy: `G:\gravity\GEO.md` — tam używaj ścieżek `geo-llmstxt`, `geo-crawlers`, itd., nie `geo-geo-*`.
