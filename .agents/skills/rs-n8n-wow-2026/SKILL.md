...
- **One-shot:** `scripts/n8n_apply_telegram_wow_suite_apr2026.py` — runs the three scripts above in order; flags ``--dry-run``, ``--vps-jwt``, ``--base-url``, optional ``--continue-on-error``, ``--json`` (machine-readable summary).
