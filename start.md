> LIVE VPS UPDATE + N8N 2.17.7 RUNNERS (2026-04-26 CET)
> VPS `185.180.207.211`: backup `/srv/backups/vps/update_20260426_100049_cursor` (APT manifest, Docker manifest, `/srv/ops-stack/compose`, n8n SQLite, Qdrant snapshots). APT upgraded Ubuntu/Docker/gcloud/crowdsec/systemd; rebooted to kernel `6.8.0-110-generic` (`reboot_required=no`, `apt_upgradable=0`). Docker images updated: n8n **2.17.7**, n8n-mcp latest, Qdrant latest, Postgres 17/15-alpine, Umami latest, SearxNG latest; `rs-support-plane-app` is local and was not pulled from registry. Fixed n8n Python task-runner warning by adding external sidecar `n8nio/runners:2.17.7`, broker local-only `127.0.0.1:5679`, n8n image pinned to `n8nio/n8n:2.17.7`. Cleanup: dangling Docker images + APT cache, `/` finalnie **72% used / 26G free**. Smoke: `auto.rs3d.pl/healthz`, n8n, n8n broker, n8n-mcp, Qdrant, ai-gateway, analytics all OK; `run_n8n_fleet_verify_vps_auto.py --definition-gate --json` -> `ok: true`.
...
> Hosting `domains/rsperformance.online/laravel/`: wgrano **auto-resolve webhook** z Public API (`N8nWorkflowWebhookPathResolver` + rozszerzony `N8nWorkflowExecuteService`), zaktualizowano `N8nWorkflowDocumentResource` (tytuły toastów) i `config/n8n.php`. Backupy: `*.bak_cursor_n8n_aw_20260412`. Komendy: `composer dump-autoload -o`, `php85 artisan config:clear`, `optimize:clear`. Smoke z serwera: `/` i `/admin/login` → **200**. Kod w repo: `ae48563` (`feature/v9-architecture-rebuild`).
> LIVE HTACCESS AI UA PARITY (2026-04-12 ~18:35 CET)
> Repo `.htaccess_remote` → hosting `public_html/.htaccess`: ModSecurity **99000/99001** + WOW `RewriteCond` dopisane o tokeny z `config/ai_agents.php`, których brakowało (**Exabot**, **OpenRouter**, **BingPreview**, **HuggingFaceBot**, **RedditBot**, **ShapBot**, **ScaleAI-Crawler**, **Together-Bot**, **cohere-crawler**, **cohere-training-data-crawler**, **hugging-face-ai**). Efekt: zaproszone agenty nie dostają **406** na canonical `/` — **302** na `https://ai.rsperformance.online/` tam gdzie polityka bramki. Backup: `.htaccess.bak_cursor_aitokens_20260412`. Smoke: `python scripts/smoke_ai_agent_catalog.py` → **0** zaproszonych z **406**.
