...
- **Hub WOW (jeden entry point)**: `G:\gravity\tools\gravity-studio-wow-runtime\setup-gravity-studio-wow.ps1` — **PHP WOW gate** (`Ensure-GravityPhpIni`) + GitHub MCP + Laravel Boost + Qdrant local + **RAG readyz** (`verify-rag-ready.ps1`); dashboard `wow.html` z **quick actions**, **fallback polami Ctrl+C**, toasts, motion, **JSON-LD**; **`serve-wow-hub.ps1`** = `http://127.0.0.1:18765/wow.html` (Clipboard odblokowany); **`-NoBrowser`** dla agentów.
