...
- **Hub WOW (jeden entry point)**: `G:\gravity\tools\gravity-studio-wow-runtime\setup-gravity-studio-wow.ps1` — **PHP WOW gate** (`Ensure-GravityPhpIni`) + GitHub MCP + Laravel Boost + Qdrant local + **RAG readyz** (`verify-rag-ready.ps1`); dashboard `wow.html` z **quick actions (clipboard)**, toasts, animacjami kart, **JSON-LD**; **`-NoBrowser`** dla agentów.
